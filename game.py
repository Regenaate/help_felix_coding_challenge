...
class FelixGame:
    ...
    def start_game(self):
        self.slow_print("So you're a therapist, and your client (Felix) wants your help on how to achieve his goals. Felix has 3 goals, and you must help him achieve each of them.")
        ...
        while not self.felix.check_goals():
            ...
            if choice.isdigit() and 1 <= int(choice) <= len(current_node.options):
                ...
                if current_node.trigger:
                    current_node.trigger(self.felix)  # Call the trigger function if it exists
                    current_node = self.story
            else:
...
girlfriend_options = [
    StoryNode('Stacy', "Stacy is a cold gold digger, so you'll have to get good at your life insurance job first. Felix won't be the happiest with her, but it will achieve his goal of companionship.", [], trigger=lambda player:  setattr(player, 'girlfriend', True)),
    StoryNode('Samantha', "Samantha will only work if Felix hasn't overcome his paranoia, because she has paranoia too. She is…kind of a psycho and can help you achieve your life insurance goal by instilling fear into your potential clients. In the end, she'll admit that she's actually", [], trigger=lambda player:  setattr(player, 'girlfriend', True)),
    StoryNode('Lizarda', "Lizarda will only work if Felix overcomes his paranoia first. Otherwise, you'll lose. But if he does, she will help him out with selling life insurance with her…soft skills 😂", [], trigger=lambda player:  setattr(player, 'girlfriend', True))
]

begning_options = [
        StoryNode('Help him get a girlfriend', 'Which girlfriend do you choose?', girlfriend_options, trigger=lambda player: setattr(player, 'girlfriend', True)),
        StoryNode('Help him improve at his job', 'Which job do you choose?', [], trigger=lambda player:  setattr(player, 'girlfriend', True)),
        StoryNode('Help him overcome his reptilian paranoia', 'Which fear do you want to overcome?', [], trigger=lambda player: setattr(player, 'girlfriend', True))
    ]

story = StoryNode("So which of Felix's goals will you help him out with first?", begning_options)
